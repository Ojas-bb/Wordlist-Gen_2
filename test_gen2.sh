...
echo -e "\n--- Running Test Cases for gen2 ---"

# Make the gen2 script executable first
chmod +x gen2_wordlist_generator.py

# 1. Basic generation
output=$(./gen2_wordlist_generator.py --min-length=2 --max-length=2 --charset="12")
expected_output=$'11\n12\n21\n22'
assert_equal "$expected_output" "$output" "Basic generation"

# 2. File output
./gen2_wordlist_generator.py --min-length=1 --max-length=1 --charset="a" -o test_output.txt
file_content=$(cat test_output.txt)
assert_equal $'a' "$file_content" "File output (-o)"

# 3. Common list inclusion
output=$(./gen2_wordlist_generator.py --common-list=common_test.txt)
...
output_non_recursive=$(./gen2_wordlist_generator.py --url="$url1")
...
output_recursive=$(./gen2_wordlist_generator.py --url="$url1" --recursive --depth=2)
...
output=$(./gen2_wordlist_generator.py --common-list=common_test.txt --min-length=1 --max-length=1 --charset="z")
expected_output=$'password\n123456\nadmin\nz'
assert_equal "$expected_output" "$output" "Combined common list and generation"

# 6. Error handling (min > max)
if ./gen2_wordlist_generator.py --min-length=3 --max-length=2 --numeric > /dev/null 2>&1; then
...
output=$(./gen2_wordlist_generator.py --min-length=1 --max-length=1 --charset="a" --prefix="pre-" --suffix="-post")
...
stderr_output=$(./gen2_wordlist_generator.py --min-length=2 --max-length=2 --charset="ab" --progress 2>&1 >/dev/null)
stdout_output=$(./gen2_wordlist_generator.py --min-length=2 --max-length=2 --charset="ab" --progress)
expected_stdout=$'aa\nab\nba\nbb'
...
output=$(./gen2_wordlist_generator.py --pattern="a?d")
expected_output=$'a0\na1\na2\na3\na4\na5\na6\na7\na8\na9'
assert_equal "$expected_output" "$output" "Pattern generation"

# 10. Pattern and charset mode mutual exclusion
if ./gen2_wordlist_generator.py --pattern="a?d" --min-length=2 > /dev/null 2>&1; then
...
echo -e "\n🎉 All gen2 tests passed successfully! 🎉"
